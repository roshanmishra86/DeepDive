...
#### Manual QA (Linux)
Browser-driven (Playwright/Chromium against `vite dev`, no DB driver — which is exactly the fresh-install state):
...
| Empty states exercised at runtime | pass — all five views render them on a fresh state; Archive CTA navigates; zero console errors/warnings |
